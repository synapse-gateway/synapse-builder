...
const getEllipseSpinner = (text) => {
  let periods = 0;
  let interval = setInterval(() => {
    if (periods === 3) periods = 1;
    else periods++;
    process.stdout.clearLine();
    process.stdout.cursorTo(0);
    process.stdout.write(`${text}${".".repeat(periods)}\x1B[?25l`);
  }, 500);
  return interval;
};
...
const run = async () => {
  ...
  // console.log("Installing AWS Copilot CLI");
  let installingAWS = getEllipseSpinner("Installing AWS Copilot CLI");
  setTimeout(() => {
    killSpinner(installingAWS);
    myEmitter.emit("END_TASK", "AWS Copilot CLI successfully installed");
  }, 5000);
